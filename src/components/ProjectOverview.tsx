 ...
 * with the joint-venture row running underneath. In the slide deck the joint venture
 * has a slide of its own, so the row is left out (`jointVenture={false}`).
 */
export function ProjectOverview({ jointVenture = true }: { jointVenture?: boolean }) {
  return (
    <section id="loyiha" aria-labelledby="loyiha-title" className="wrap scroll-mt-20">
      ...
      <Reveal timing={{ notBefore: 0.9 }} className="rounded-[1.75rem] border-[1.5px] border-dashed border-navy/35 p-4 sm:p-6 lg:p-7 deck:p-6">
